...
**Vaka**: [#76 kafe-gorevlisi](https://github.com/Mavrikant/games/pull/76), [#77 lazer-ayna](https://github.com/Mavrikant/games/pull/77), [#78 kor-tus](https://github.com/Mavrikant/games/pull/78) JSON metadata'sında `"thumbnail": "<slug>.svg"` yazıyordu ama `public/thumbs/<slug>.svg` dosyaları yoktu. `Card.astro` alanı koşulsuz okuyup `<img src="...">` render ettiği için arşiv kartında **broken image** çıktı; OG paylaşımı da default'a düştü. Build geçti çünkü Astro thumbnail dosyasının varlığını doğrulamıyor (alan opsiyonel string).
**Pattern**: Content schema'da opsiyonel bir alan dolduruluyor ama referans verdiği harici asset (SVG, image, font) repo'da yok. Build referansları statik olarak takip etmediği için dangling. Browser request 404 alır, görsel kırılır.
**Tespit**: `scripts/audit-games.mjs` CI gate'i her JSON'un `thumbnail` alanını `public/thumbs/` ile çapraz kontrol eder; eksikse exit 1. Manuel: `/games/` arşivini aç ve yeni oyunun kartında broken-img placeholder'ı var mı bak; F12 Network'te 404.
**Önlem**: İki seçenek: ya `thumbnail` alanını JSON'dan **çıkar** (Card placeholder harf gösterir, kart yine çalışır) ya da `public/thumbs/<slug>.svg`'yi **gerçekten oluştur**. Şüphedeysen alanı çıkar; thumbnail polish-PR'ı olarak sonra eklenebilir. Yeni oyun scaffold'u (`npm run new-game`) `thumbnail` alanını JSON'a koymuyor — agent prompt'u manuel eklerse dosyayı da oluşturmalı.
