...
önünde tek bir **Supabase** projesi durur: Postgres + Auth (Google OAuth)
...
Statik frontend (base /games/)                      Supabase
  /siralama  ──OAuth login──▶ Auth (Google)
...
2. **Google OAuth uygulaması** oluştur (Google Cloud Console → Credentials →
   OAuth client ID → **Web application**). **Authorized redirect URI** = Supabase
   callback'i: `https://<PROJECT-REF>.supabase.co/auth/v1/callback` (uygulamanın
   URL'i DEĞİL). Oluşan Client ID + Secret'ı Supabase → Authentication →
   Providers → Google'a gir ve etkinleştir.
3. **URL Configuration** (Supabase → Authentication → URL Configuration):
   **Site URL** = `https://karaman.dev/games/siralama/`; **Redirect URLs**'e
   `https://karaman.dev/games/siralama/` (ve dev için
   `http://localhost:4321/games/siralama/`) ekle. (Site URL yanlışsa giriş
   sonrası `localhost:3000`'e düşer.)
4. **SQL** (aşağıdaki şema + RLS + RPC + trigger).
...
-- Her yeni kullanıcı için profiles satırını OAuth profilinden otomatik oluştur.
-- (Pano isimleri buradan gelir; ayrıca aşağıdaki scores->profiles FK'si için şarttır.)
...
-- user_id, auth.users yerine profiles'a bağlanır: PostgREST'in pano okumasındaki
-- `profiles(display_name)` embed'i bu doğrudan FK'yi gerektirir (profiles.user_id
-- zaten auth.users'a bağlı; zincir korunur).
...
- **Açık**: Supabase projesini kur, `.env`'i doldur, `/siralama`'da Google
